...

{
  ...
  config = mkIf config.home.profiles.desktop.enable (mkMerge [
    {
    ...
    (mkIf config.home.profiles.desktop.browsers.firefox {
      programs.firefox = {
        enable = true;
        profiles.default = {
          id = 0;
    ...
    (mkIf config.home.profiles.desktop.browsers.chromium {
      programs.chromium = {
        enable = true;
      };
    })
    {
      ...
      home.file = {
        ...
        "Vault/.keep" = mkIf (
          config.home.profiles.power-user.enable && config.home.profiles.power-user.productivity.enable
        ) { text = ""; };
