...
#                 Audiobookshelf, Calibre, Stirling-PDF, Actual Budget,
#                 Hoarder, Dawarich
###############################################################################
let
  das1 = "/mnt/das1";
in
{
  ...
  #
  # Replaces: immich_server + immich_machine_learning +
  #           immich_postgres + immich_redis (4 containers → 1 native service)
  # ===========================================================================
  services.immich = {
    enable = true;
    host = "127.0.0.1";
    ...
    accelerationDevices = [ "/dev/dri/renderD128" ];  # Intel GPU for ML
    ...
    database = {
      createDB = false;  # Database already created by databases.nix
      host = "/run/postgresql";
      name = "immich";
      user = "immich";
    };

    # Use the shared Redis instance (DB 0)
    redis = {
      host = "127.0.0.1";
      port = 6379;
      dbIndex = 0;
  ...
  #
  # Replaces: vaultwarden container
  # ===========================================================================
  ...
  # vaultwarden needs a postgres database. Add it to postgres config.
  # NOTE: This extends the databases.nix postgres config.
  # Since ensureDatabases is a list, we use a separate approach:
  # The vaultwarden NixOS module normally creates its own DB via sqlite.
  # For postgres, we need the DB pre-created — add it manually in
  # databases.nix if desired, or let vaultwarden init it.

  # ===========================================================================
  # SYNCTHING — File Synchronization (native NixOS module)
  #
  # Replaces: syncthing container
  # ===========================================================================
  services.syncthing = {
    enable = true;
    user = "jpolo";
    ...
    settings = {
      devices = {
        # Add your syncthing devices here:
        # "pixel-9a" = { id = "DEVICE-ID-HERE"; };
      };
      folders = {
        "obsidian" = {
          path = "${das1}/obsidian";
  ...
  virtualisation.oci-containers.containers = {
    ...
    #
    # NOTE: AIO manages its own internal containers and needs docker socket.
    # If podman docker compat doesn't work, use the native services.nextcloud.
    # =========================================================================
    nextcloud-aio = {
    ...
    firefly-iii = {
      ...
      environment = {
        ...
        APP_ENV = "local";
        APP_KEY = "___SECRET_MANAGED___";  # 32-char base64
        STATIC_CRON_TOKEN = "___SECRET_MANAGED___";  # 32-char
        TZ = "Europe/Madrid";
      };
    };
    ...
    calibre = {
      ...
      ports = [
        "16080:8080"   # Desktop GUI
        "16081:8081"   # Content Server (OPDS)
      ];
      environment = {
        PUID = "1000";
        PGID = "1000";
    ...
    hoarder = {
      ...
      ports = [ "3001:3000" ];  # 3001 to avoid Grafana (native) on 3000
      volumes = [
