...
let
  ...
  # NordVPN credentials via sops-nix.
  # Before deploying, add to secrets/secrets.yaml:
  #   vpn:
  #     user: "your-nordvpn-user"
  #     password: "your-nordvpn-password"
  # Then encrypt: sops secrets/secrets.yaml

  # sops-nix secrets for VPN credentials
  ...
  # Environment file for gluetun (populated from sops secrets at boot)
...
in
{
  ...
  # Plex runs as 'plex' user, but media files are owned by jpolo.
  # Allow plex to read media by adding it to the 'users' group.
  users.users.plex.extraGroups = [ "users" ];

  # Plex needs access to media directories on USB drives.
  # systemd mount propagation for /mnt/das1 and /mnt/elements
  ...
  virtualisation.oci-containers.containers = {
    ...
    #
    # qbittorrent and jackett share this container's network namespace
    # so all their traffic goes through the VPN.
    # =========================================================================
    gluetun = {
      ...
      # VPN credentials come from sops-nix generated env file (not world-readable)
      environmentFiles = [
    ...
    qbittorrent = {
      ...
      extraOptions = [
        "--restart=always"
        "--network=container:gluetun"  # VPN sidecar pattern
      ];
      environment = lsioEnv // {
        WEBUI_PORT = "8080";
      };
