...
# OCI containers: cAdvisor, Scrutiny, Homepage, Gotify
###############################################################################
let
...
in
{
  ...
  services.prometheus = {
    enable = true;
    port = 9090;
    listenAddress = "127.0.0.1";  # proxied via nginx

    # 7 days retention, 30s scrape interval
    retentionTime = "168h";
    globalConfig = {
      scrape_interval = "30s";
    ...
    # Alerting rules (conservative for 16GB RAM home server)
    rules = [
      ''
        groups:
        - name: system
  ...
  services.grafana = {
    ...
    settings = {
      ...
      security = {
        admin_user = "admin";
        # Grafana passwords are set in Nix config (ends up in /nix/store).
        # For a home server behind Tailscale, change after first login via UI.
        # For a fully hardened setup, use GF_SECURITY_ADMIN_PASSWORD__FILE env var.
  ...
  #
  # If memory is tight on 16 GB, reduce or disable Loki.
  # It's useful but not essential — journalctl is always available.
  # ---------------------------------------------------------------------------
  services.loki = {
    enable = true;
    configuration = {
  ...
  # ---------------------------------------------------------------------------
  # PROMTAIL — Ships logs to Loki from journald and container logs
  # ---------------------------------------------------------------------------
  services.promtail = {
    enable = true;
    configuration = {
  ...
  # Add Uptime Kuma to nginx routes (see networking.nix for the location block)
  # Access at: http://apollo.<tailnet>/uptime
  ...
  # Enable nginx stub_status for exporter
  ...
  virtualisation.oci-containers.containers = {
    ...
    # (runs as container since services.cadvisor depends on docker.service)
    # =========================================================================
    cadvisor = {
    ...
    # =========================================================================
    # HOMEPAGE — Dashboard (default landing page)
    # =========================================================================
    homepage = {
    ...
    # (Optional: remove once comfortable with NixOS declarative management)
    # =========================================================================
    portainer = {
