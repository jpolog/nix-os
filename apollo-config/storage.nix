...
###############################################################################
{
  ...
  # WD Elements 5.5TB — secondary storage (downloads, books, etc.)
  ...
  #
  # systemd-tmpfiles creates these on boot (like /var/lib/* but persistent).
  # ---------------------------------------------------------------------------
  ...
  in [
    ...
    "${d} /var/lib/mongodb      ${z} mongodb mongodb   ${r}"
    ...
    "${d} /var/lib/penpot              ${z} root root ${r}"
    "${d} /var/lib/open-webui          ${z} root root ${r}"

    # Media data directories (mounted from USB drives — tmpfiles ensures they exist)
    # These are on /mnt/das1 which is mounted at boot
